                ...
                <div class="formulario__grupo-input"></div>
                 <select id="rol_id" name="rol_id" class="formulario__input">
                    @foreach ($roles as $rol)
                    	<option value="{{$rol->rol_id}}">{{$rol->description}}</option>
                    @endforeach
               </select>
               </div>

            <!-- Grupo: Liga para iniciar sesión -->
           <div>
                   <class="links"> ¿ya tienes una cuenta? &nbsp; &nbsp; &nbsp;<a href="{{route('login')}}" class="links">Iniciar sesión</a>
            </div>
